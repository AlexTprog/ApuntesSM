...
double solveByTrapezoidMethod(string function, double n, double a, double b) {
  // resolver por hilos y regresar el resultado

  double intevalo = (double)(b - a) / n;
  double result = 0;
  double i = a;
  do {
    result += (double)solveIntegral(function, i, i + intevalo);
    i = i + intevalo;
    if (round(i) == b && roundAux(i) == 9) {
      break;
    }
  } while (i < b);
...
double calculateError(string function, double n, double a, double b) {
  ...
  double x = solveByTrapezoidMethod(function, n + 1, a, b);
  double y = solveByTrapezoidMethod(function, n, a, b);
...
int main() {
  ...
  // hilos para n impare y n pares
  int numThreads = 2;
  omp_set_num_threads(numThreads);
  double nImpar = 0;
  double nPar = 0;
  double nImparError = 0;
  double nParError = 0;
  double nImparErrorAnterior = 0;
  double nParErrorAnterior = 0;
  double nImparErrorActual = 0;
  double nParErrorActual = 0;
  double nImparErrorDeseado = 0;
  double nParErrorDeseado = 0;
#pragma omp parallel
  {
    int id = omp_get_thread_num();
    ...
    if (id == 0) {
      nImpar = 1;
      nImparError = 1;
      nImparErrorAnterior = 1;
      nImparErrorActual = 1;
      nImparErrorDeseado = 1;
      while (nImparError > error_deseado) {
        nImparErrorAnterior = nImparErrorActual;
        nImparErrorActual = calculateError(f, nImpar, a, b);
        nImparErrorDeseado =
            abs((double)(nImparErrorActual - nImparErrorAnterior));
        nImparError = nImparErrorDeseado;
        nImpar += 2;
      }
    } else {
      nPar = 2;
      ...
      nParErrorActual = 1;
      nParErrorDeseado = 1;
      while (nParError > error_deseado) {
        nParErrorAnterior = nParErrorActual;
        nParErrorActual = calculateError(f, nPar, a, b);
        nParErrorDeseado = abs((double)(nParErrorActual - nParErrorAnterior));
        nParError = nParErrorDeseado;
        nPar += 2;
      }
    }
  ...
  if (nImpar < nPar) {
    cout << calculateError(f, nPar, a, b) << endl;
    cout << solveByTrapezoidMethod(f, nPar, a, b) << endl;
    cout << nPar << endl;
  } else {
    cout << calculateError(f, nImpar, a, b) << endl;
    cout << solveByTrapezoidMethod(f, nImpar, a, b) << endl;
    cout << nImpar << endl;
  }
  return 0;
}